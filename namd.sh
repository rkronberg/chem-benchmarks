...
if [[ $SLURM_GPUS_ON_NODE -gt 0 ]]; then
    ...
    if [[ $HOSTNAME == "rg"* ]]; then
        if [[ $SLURM_GPUS_ON_NODE -eq 1 ]]; then
            SRUN_OPTS="--cpus-per-task 72"
            ARGS="+p 72 +setcpuaffinity +devices 0 $1"
        fi
        if [[ $SLURM_GPUS_ON_NODE -eq 2 ]]; then
            SRUN_OPTS="--cpus-per-task 133"
            ARGS="+p 135 +pmepes 63 +setcpuaffinity +devices 0,1 $1"
        fi
        if [[ $SLURM_GPUS_ON_NODE -eq 4 ]]; then
            SRUN_OPTS="--cpus-per-task 244"
            ARGS="+p 253 +pmepes 37 +setcpuaffinity +devices 0,1,2,3 $1"
        fi
    fi
fi
